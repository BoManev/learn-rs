pub trait IteratorExt: Iterator {
    fn call_flatten(self) -> Flatten<Self>
    where
        Self: Sized,
...
mod tests {
    ...
    #[test]
    fn ext() {
        assert_eq!(vec![vec![0, 1]].into_iter().call_flatten().count(), 2);
    }
}

/*
* let vs = vec![1, 2, 3];
* for v in vs -> consumes vs, owned v
* for v in vs.iter() -> borrows vs, & to v
* for v in &vs -> same as vs.iter()
*/
